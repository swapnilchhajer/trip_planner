...
class Place(BaseModel):
    ...
    category: str = Field(description="Category of the sight (e.g., Landmark, Museum, Nature).")
...
class TransitOption(BaseModel):
    mode: str = Field(description="Mode of transportation (e.g., Flight, High-Speed Train, Express Bus, Car Rental).")
...
class TransportationOptionsResponse(BaseModel):
    ...
    transportation_options: List[TransitOption] = Field(description="List of transit options between origin and destination.")
...
class PricingBreakdown(BaseModel):
    accommodation: AccommodationItem = Field(description="Accommodation budget details.")
    meals_and_food: CostItem = Field(description="Food and meals budget details.")
    activities_and_sightseeing: CostItem = Field(description="Sightseeing and activities budget details.")
    local_transportation: CostItem = Field(description="Local transit budget details.")
    miscellaneous_contingency: ContingencyItem = Field(description="Safety margin and contingency details.")
...
class ErrorResponse(BaseModel):
    ...
    error_code: str = Field(description="Machine-readable error identifier for programmatic parsing.")
    message: str = Field(description="Human-readable explanation of what went wrong.")
    recovery_instructions: str = Field(description="Guided instructions instructing the LLM how to recover and what clarification to ask the user.")
...
def get_places_to_visit(destination: str) -> Union[PlacesToVisitResponse, ErrorResponse]:
    ...
    database = {
        ...
        "france": [
            ...
            {"name": "Chamonix-Mont-Blanc", "description": "A world-renowned alpine resort area at the foot of Mount Fuji / Mont Blanc, ideal for skiing and hiking.", "category": "Nature / Adventure", "recommended_duration": "1-2 days"}
        ]
    }
...
def get_transportation_options(origin: str, destination: str) -> Union[TransportationOptionsResponse, ErrorResponse]:
    ...
    elif ("new york" in orig_lower or "san francisco" in orig_lower or "los angeles" in orig_lower) and ("tokyo" in dest_lower or "japan" in dest_lower):
        options = [
            TransitOption(
...
def get_overall_pricing_estimate(destination: str, days: int, travel_style: str) -> Union[PricingEstimateResponse, ErrorResponse]:
